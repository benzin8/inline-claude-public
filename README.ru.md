...
- 🪪 **Поиск инфы о собеседнике** — на первом сообщении от нового контакта запрашивает его профиль (имя, юзернейм, био, телефон если виден) через юзербота и кэширует для контекста
...
- ⭕ **Кружки (video note)** — Claude смотрит присланные кружки через skill `watch` (нужен плагин [claude-video](https://github.com/bradautomates/claude-video)) и описывает/расшифровывает их
...
| Плагин `telegram@claude-plugins-official`, включённый в той же сессии и настроенный на **бот-бридж** | Без него тригер некому подхватить — юзербот просто шлёт обычное сообщение боту-бриджу, а видит его сессия только через канал этого плагина | ✅ |
| ffmpeg | Расшифровка голосовых | Только для голосовых |
| Плагин [claude-video](https://github.com/bradautomates/claude-video) (skill `watch`) | Просмотр кружков (video note) | Только для кружков |
...
| `CLEANUP_BRIDGE_MSG` | Удалять сырое сообщение-тригер `[[ic:...]]` из чата с бриджем после того, как Клод ответил, — чтобы не копилось | `true` |
...
| Скрипт | Назначение |
|---|---|
| `get_biz_history.py <chat_id> [limit]` | История чата из SQLite (плюс кэш инфы о собеседнике, если уже получена) |
| `get_contact_info.py <chat_id>` | Забирает профиль пользователя через юзербота, печатает JSON (вызывается автоматически при первом контакте) |
| `delete_message.py <target> <message_id>` | Удаляет сообщение, отправленное юзерботом (используется автоматически для чистки сообщений-триггеров) |
...
Эти файлы — как пароль: у кого они есть, тот может читать твой Telegram и слать
сообщения от твоего имени:

- **`~/.claude/userbot/*.session`** — сессия юзербота. Полный доступ к твоему аккаунту,
  без запроса 2FA.
- **`.env`** (токен бота, owner id) — у кого токен, тот управляет твоим ботом.

Оба уже в `.gitignore` и никогда не попадают в публикуемый npm-пакет (проверь сам:
`npm pack --dry-run`) — просто не копируй/не пересылай их никуда (в чат, скриншот,
issue).
