...
⚠️ Этот файл = пароль от твоего Telegram-аккаунта (без 2FA). Никому не отправляй,
не коммить, не публикуй — он уже в `.gitignore`, но проверяй сам при переносе на
другую машину.
...
Замене `/path/to/` на реальный путь (обычно `~` или `/Users/имя`).

**Важно:** этого одного MCP-сервера НЕДОСТАТОЧНО, чтобы триггеры реально доходили до
сессии. Юзербот доставляет [[ic:...]] тем, что просто шлёт обычное сообщение боту
из `BRIDGE_TARGET` — а увидит его сессия, только если в этом же проекте включён и
настроен официальный плагин `telegram@claude-plugins-official` (канал), привязанный
именно к боту из `BRIDGE_TARGET`. Без этого плагина сообщение придёт боту, но никто
его не прочитает и вопрос повиснет без ответа.

Включи плагин в Claude Code (`/plugin` → `telegram` → `claude-plugins-official` →
Install), укажи ему токен того же бота, что в `BRIDGE_TARGET`, и держи именно эту
сессию/проект открытой — она и будет "слушать" тригеры.
